...
def test_s3_mapping_bucket():
    with mock_s3():
        ...
        with open(os.path.join(os.path.dirname(__file__), 'fixtures', 'WfEngineRawForTAKinesisFire-rl7PaeLbt4Vx-1-2020-06-25-19-20-02-cd4559fa-5b3c-4b7e-8d8a-cfbbc48071dd'), 'r') as f:
            body = f.read()
        s3.create_bucket(Bucket='raw-feed')
        assert s3.put_object(Bucket='raw-feed',
                Key='2020/06/25/19/WfEngineRawForTAKinesisFire-rl7PaeLbt4Vx-1-2020-06-25-19-20-02-cd4559fa-5b3c-4b7e-8d8a-cfbbc48071dd',
                Body=body)['ResponseMetadata']['HTTPStatusCode'] is 200
...
def test_parse_csv_mapping():
    ...
    with mock_s3():
        ...
        with open(os.path.join(os.path.dirname(__file__), 'fixtures', 'WfEngineRawForTAKinesisFire-rl7PaeLbt4Vx-1-2020-06-25-19-20-02-cd4559fa-5b3c-4b7e-8d8a-cfbbc48071dd'), 'r') as f:
            body = f.read()
        s3.create_bucket(Bucket='raw-feed')
        s3.put_object(Bucket='raw-feed',
                Key='2020/06/25/19/WfEngineRawForTAKinesisFire-rl7PaeLbt4Vx-1-2020-06-25-19-20-02-cd4559fa-5b3c-4b7e-8d8a-cfbbc48071dd',
                Body=body)
    response = parse_csv_for_mapping('1234567890123456789012345', '2020-06-26T19:05:16.785Z', doc_topics_file_name=os.path.join(os.path.dirname(__file__), 'fixtures/doc-topics.csv'))
...
def test_parse_csv_for_topics():
    ...
    response = parse_csv_for_topics('1234567890123456789012345', '2020-06-26T19:05:16.785Z',topic_terms_file_name=os.path.join(os.path.dirname(__file__), 'fixtures/topic-terms.csv'))
...
def test_publish_on_event_bridge_with_parse_file():
    ...
    with stubber:
        ...
        response = event_bridge_client.put_events(
            Entries = [{
                'EventBusName': os.environ['EVENT_BUS_NAME'],
                'Detail': json.dumps(parse_csv_for_topics('1234567890123456789012345', '2020-06-26T19:05:16.785Z',topic_terms_file_name=os.path.join(os.path.dirname(__file__), 'fixtures/topic-terms.csv'))),
                'Source': os.environ['TOPICS_EVENT_NAMESPACE'],
                'DetailType': 'topics'
            }]
        )
